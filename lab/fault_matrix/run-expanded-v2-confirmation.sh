...
output="${repository}/artifacts/demand-conditioned-v2/expanded-confirmation"
captures="${output}/cases"
model="${repository}/artifacts/demand-conditioned-v2/model.json"
experiments="${lab_directory}/experiments_v2_expanded"
protocol="${lab_directory}/v2-expanded-confirmation-protocol.json"
protocol_verifier="${lab_directory}/verify_v2_protocol.py"
compose=(
  docker compose
  --project-name quantis-v2-expanded
  --file "${lab_directory}/compose.yaml"
)
case_files=("${experiments}"/*.json)
...
.venv/bin/python -m quantis_core evaluate-demand-conditioned-matrix \
  --captures-directory artifacts/demand-conditioned-v2/expanded-confirmation/cases \
  --manifests-directory lab/fault_matrix/experiments_v2_expanded \
  ...
  --confirmation-protocol lab/fault_matrix/v2-expanded-confirmation-protocol.json \
  --preregistered-git-commit "${preregistered_git_commit}" \
  --output artifacts/demand-conditioned-v2/expanded-confirmation
