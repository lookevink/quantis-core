...
output="${repository}/artifacts/jepa-world-model-v3/graph-observability-pilot-v1"

cd "${repository}"
.venv/bin/python \
  "${lab_directory}/run_graph_observability_pilot.py" \
  ...
  --source-assessment "${source}/assessment/confirmation.json" \
  --output "${output}"
